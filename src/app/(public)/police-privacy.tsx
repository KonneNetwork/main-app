...
interface PolicePrivacyProps {
  policy: boolean;
  setPolicy: React.Dispatch<React.SetStateAction<boolean>>
  setTerms: React.Dispatch<React.SetStateAction<boolean>>
}

export default function PolicePrivacy({ policy, setPolicy, setTerms }: PolicePrivacyProps) {
  return (
    <Modal visible={policy} presentationStyle='fullScreen' animationType='fade' style={{ flex: 1 }}>
      <ScrollView className='flex-grow w-full' contentContainerStyle={{ flexGrow: 1 }}>
        <SafeAreaView className='flex-1 bg-white p-6'>
          <StatusBar />
          <View className='flex-row  items-center'>
            <Ionicons name="chevron-back-outline" size={32} color="black" onPress={() => setPolicy(false)} />
          ...
          <View className='flex-row flex-1 items-end justify-between pt-8 justify-self-end'>
            <Text className='text-sm color-[#666666] font-inter-400'> © 2023-2024 Konne S.A </Text>
            <TouchableOpacity onPress={() => { setPolicy(false); setTerms(true); }}>
