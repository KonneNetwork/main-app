...
export default function TermsUse({ terms, setTerms, setPolicy }: termsUseProps) {
  return (
    <Modal visible={terms} presentationStyle='fullScreen' animationType='fade' style={{ flex: 1 }}>
      <ScrollView className='flex-grow w-full ' contentContainerStyle={{ flexGrow: 1 }} >
        <SafeAreaView className='flex-1 bg-white p-6'>
          <StatusBar />
          <View className='flex-row  items-center'>
            <Ionicons name="chevron-back-outline" size={32} color="black" onPress={() => setTerms(false)} />
          ...
          <View className='flex-row flex-1 items-end justify-between'>
            ...
            <TouchableOpacity className='flex-wrap' onPress={() => { setTerms(false); setPolicy(true) }}>
              <Text className='text-base color-[#33586C] font-inter-700' style={{ flexWrap: "wrap" }}> POLÍTICA DE PRIVACIDADE </Text>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      </ScrollView>
